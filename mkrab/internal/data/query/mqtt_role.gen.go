...
func newRole(db *gorm.DB, opts ...gen.DOOption) role {
	...
	_role.ACLRules = roleHasManyACLRules{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("ACLRules", "model.ACLRule"),
	}
...
func (r role) clone(db *gorm.DB) role {
	...
	r.ACLRules.db = db.Session(&gorm.Session{Initialized: true})
	r.ACLRules.db.Statement.ConnPool = db.Statement.ConnPool
	return r
}

func (r role) replaceDB(db *gorm.DB) role {
	r.roleDo.ReplaceDB(db)
	r.ACLRules.db = db.Session(&gorm.Session{})
	return r
}

type roleHasManyACLRules struct {
	db *gorm.DB

	field.RelationField
}

func (a roleHasManyACLRules) Where(conds ...field.Expr) *roleHasManyACLRules {
	if len(conds) == 0 {
		return &a
	}
...
func (a roleHasManyACLRules) WithContext(ctx context.Context) *roleHasManyACLRules {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a roleHasManyACLRules) Session(session *gorm.Session) *roleHasManyACLRules {
	a.db = a.db.Session(session)
	return &a
}

func (a roleHasManyACLRules) Model(m *model.Role) *roleHasManyACLRulesTx {
	return &roleHasManyACLRulesTx{a.db.Model(m).Association(a.Name())}
}

func (a roleHasManyACLRules) Unscoped() *roleHasManyACLRules {
	a.db = a.db.Unscoped()
	return &a
}

type roleHasManyACLRulesTx struct{ tx *gorm.Association }

func (a roleHasManyACLRulesTx) Find() (result []*model.ACLRule, err error) {
	return result, a.tx.Find(&result)
}

func (a roleHasManyACLRulesTx) Append(values ...*model.ACLRule) (err error) {
...
func (a roleHasManyACLRulesTx) Replace(values ...*model.ACLRule) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleHasManyACLRulesTx) Delete(values ...*model.ACLRule) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleHasManyACLRulesTx) Clear() error {
	return a.tx.Clear()
}

func (a roleHasManyACLRulesTx) Count() int64 {
	return a.tx.Count()
}

func (a roleHasManyACLRulesTx) Unscoped() *roleHasManyACLRulesTx {
	a.tx = a.tx.Unscoped()
	return &a
}
